...
CSV.open("#{$run_date_csv_log}_#{$service}_validation_Preprod.csv","w+") do |cs|
    cs << ["SM_Title", "rel_yr", "Showtime_SM_ID", "SM_Map_Status","Duplicate_PX_id","Blind_ID","OZ_SM_ID","Blind_Status",
...
def total_series(service)
    ...
    arr_invalid = []
    arr = []
    series = $client.query("SELECT * FROM #{$db_table} where item_type='tv_show';")
    series.each do |series_details|
        series_details = series_details.to_json
        series_details = JSON.parse(series_details)
        puts series_details
        if series_details["expiry_date"].nil? || series_details["expiry_date"] > $run_date
            arr << series_details["source_program_id"]
        end    
    end
    arr = arr.uniq
    $log.info "total no.of series available : #{arr.length}"
    arr.each do |series_id|
        series_validity = $client.query("SELECT count(*) FROM #{$db_table} where item_type='episode' and series_id='#{series_id}' and (expiry_date is null or expiry_date > '#{$run_date}');")
        $log.info "-------------------------------------------------------------------------"
...
valid_series.each do |series_id|
    ser_id = series_id
    $log.info ser_id
    query_series_id = $client.query("select source_program_id,title,year from #{$db_table} where item_type='tv_show' and source_program_id='#{ser_id}'")
    #create new array                            
    ne = Array.new
    query_series_id.each do |details|
        begin
            ...
            rel_year_totest = ne[0]["year"]
            rel_year_totest = rel_year_totest.to_i
            show_type_totest = "SM"
            launch_id_totest = ne[0]["source_program_id"]
            ...
            if rel_year_totest == 0
                ...
                mon_sns = []
                mon_sns =$client.query("select distinct(season_number) from #{$db_table} where (item_type='episode' and series_id='#{launch_id_totest}')")  
                mon_eps =$client.query("select distinct(episode_number) from #{$db_table} where (item_type='episode' and series_id='#{launch_id_totest}')")  
                $log.info "showtime seasons are #{mon_sns}"
                $log.info "showtime episodes are #{mon_eps}"
                ep_ar = Array.new
                sn_ar = Array.new
                ...
                mon_relyr = $client.query("select year from #{$db_table} where item_type='episode' and series_id='#{launch_id_totest}' and season_number='#{temp_seasonno}' and episode_number='#{temp_episodeno}';")
                mon_relyr.each do |ry|
                    ry = ry.to_json
                    ry = JSON.parse(ry)
                    temp_relyr = (ry["year"]).to_i
            ...
            if rel_year_totest > 0
                ...
                if blind_ingest_status[0] == "Pass"
                    ...
                    if !search_result.empty?
                            ...
                            if ((program_longtitle == title_totest_m) || (program_originaltitle == title_totest_m) || (alias_match_flag == 1))
                                ...
                                if obj["object"]["show_type"] == "SM"
                                    ...
                                    if (obj["object"]["release_year"] == rel_year_totest) || (obj["object"]["release_year"] == rel_year_totest-1) || (obj["object"]["release_year"] == rel_year_totest+1)
                                        ...
                                        dist_seasons_q = $client.query("select distinct(season_number) from #{$db_table} where item_type='episode' and series_id='#{ser_id}' and (expiry_date is null or expiry_date > '#{$run_date}');")
                                        dist_seasons = []                                        
                                        ...
                                        dist_seasons.each do |eps|
                                            ...
                                            mon_ep = $client.query("select title,source_program_id,season_number,episode_number from #{$db_table} where season_number='#{eps}' and item_type='SE' and series_id='#{ser_id}' and (expiry_date is null or expiry_date > '#{$run_date}');")
                                            episodes_season = []
                                            mon_ep.each do |des|
                                                des = des.to_json
                                                des = JSON.parse(des)
                                            ...
                                            $log.info "episodes from showtime db are #{episodes_season}"
                                            ...
                                            episodes_season.each do |start|
                                                begin
                                                    ...
                                                    episode_ott_to_test = start["source_program_id"]
                        ...
                        mon_ep = $client.query("select source_program_id from #{$db_table} where item_type='episode' and series_id='#{ser_id}' and (expiry_date is null or expiry_date > '#{$run_date}');")
                        episodes_series = []
                        mon_ep.each do |des|
                            des = des.to_json
                            des = JSON.parse(des)
