...
CSV.open("#{run_date}Headrun_validation_Preprod.csv","w+") do |cs|
    cs << ["SM_Title", "rel_yr", "FOX_SM_ID", "SM_Map_Status","Duplicate_PX_id","Blind_ID","OZ_SM_ID","Blind_Status",
...
valid_series = total_series($service);
...
valid_series.each do |series_id|
    ser_id = series_id
    query_series_id = $collection.find({"item_type":"tvshow","service":$service,"id":"#{ser_id}"})
    ...
    query_series_id.each do |details|
        begin
            ...
            if rel_year_totest == 0
                ...
                mon_sns = []
                mon_sns = $collection.distinct(("season_number"),{"item_type":"episode","service":$service,"series_id":"#{launch_id_totest}"})
                ...
                mon_relyr = $collection.find({"item_type":"episode","service":$service,"series_id":"#{launch_id_totest}","season_number":"#{temp_seasonno}"}).projection({"_id":0,"release_year":1}).limit(1)
            ...
            if rel_year_totest > 0
                ...
                if blind_ingest_status[0] == "Pass"
                    ...
                    if !search_result.empty?
                            ...
                            if ((program_longtitle == title_totest_m) || (program_originaltitle == title_totest_m) || (alias_match_flag == 1))
                                ...
                                if obj["object"]["show_type"] == "SM"
                                    ...
                                    if (obj["object"]["release_year"] == rel_year_totest) || (obj["object"]["release_year"] == rel_year_totest-1) || (obj["object"]["release_year"] == rel_year_totest+1)
                                        ...
                                        dist_seasons_q = $collection.distinct(("season_id"),{"item_type":"episode","service":$service,"series_id":"#{ser_id}"})
                                        dist_seasons = []
                                        ...
                                        dist_seasons.each do |eps|
                                            ...
                                            mon_ep = $collection.find({"item_type":"episode","series_id":"#{ser_id}","service":$service,"season_id":"#{eps}"}).projection({"_id":0,"episode_title":1,"season_number":1,"episode_number":1,"id":1,})
                                            episodes_season = []
                                            mon_ep.each do |des|
                                                des = des.to_json
                                                des = JSON.parse(des)
                        ...
                        mon_ep = $collection.find({"item_type":"episode","service":$service,"series_id":"#{ser_id}"}).projection({"_id":0,"id":1})
                        episodes_series = []
                        mon_ep.each do |des|
                            des = des.to_json
                            des = JSON.parse(des)
