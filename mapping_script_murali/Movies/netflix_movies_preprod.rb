...
def voice_search_pagination(search_term,tab)
  ...
  retry_cnt = 3
    begin
    json_body = JSON.parse(RestClient.get("https://preprod.caavo.com/v3/voice_search?ott=true&service=netflixusa&aliases=true&q=#{URI.escape("#{search_term}")}", {:authorization => 'Token token=ddfa7110e3562b2a87314d383e31d9af5d8283bfe08d74940bc54c54858b0d11',:user_agent => 'Caavo_Fyra_v1.1.199'}))
  ...
  if results_array.length > 0
    ...
    if ott_search_index != nil
      ...
      if results_array[ott_search_index].key?("next_page_params")
        ...
        final_next_url = nil
        final_next_url = "/v3/voice_search?query=" + "#{query}" +"&search_id=" + "#{search_id}" + "&page=" + "#{page}" + "&filter=" + "#{filter}" + "&ott=true" + "&service=netflixusa" + "&aliases=true"
    ...
    while (next_key)
      ...
      if (json_body["results"]).length > 0
        ...
        if json_body["results"][0].key?("next_page_url")
          ...
          url = json_body["results"][0]["next_page_url"] + "&ott=true" + "&service=netflixusa" + "&aliases=true"
          next_key = true
        else
           next_key = false
        end 
...
def check_projectx_ingested(vu_ser_id)
    retry_cnt = 3
    begin
        pjx_body = JSON.parse(RestClient.get("http://34.231.212.186:81/projectx/mappingfromsource?sourceIds=#{vu_ser_id}&sourceName=Netflixusa&showType=MO", {:authorization => 'Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3',:user_agent => 'Caavo_Fyra_v1.1.199',:host => '54.175.96.97:81'}))
        if !pjx_body.empty?
            ingestion_status = []
...
def check_projectx_duplicate(vu_ser_id)
    retry_cnt = 3
    begin
        dup_body = JSON.parse(RestClient.get("http://34.231.212.186:81/projectx/duplicate?sourceId=#{vu_ser_id}&sourceName=Netflixusa&showType=MO", {:authorization => 'Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3',:user_agent => 'Caavo_Fyra_v1.1.199',:host => '54.175.96.97:81'}))
        if !dup_body.empty?
...
def check_ott_deleted(vu)
    retry_cnt = 3
    begin
        del_status = JSON.parse(RestClient.get("http://34.231.212.186:81/projectx/#{vu}/Netflixusa/isDeletedOttprojectx", {:authorization => 'Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3',:user_agent => 'Caavo_Fyra_v1.1.199',:host => '54.175.96.97:81'}))
        if !del_status.empty?
            return "true"
        else
            return "false"
...
CSV.open("#{run_date}nteflix_preprod.csv","w+") do |cs|
	cs << ["NF_title","rel_yr","NF_id","resp_code","rel_yr_status","matched_status","oz_title","rel_yr","program_id","blind_id","search_status","links_status","nteflix_id","rovi_id"]
end
CSV.open("#{run_date}movie_notmapped_preprod.csv","w+") do |cs|
end
CSV.open("#{run_date}nteflix_movies_empty_search_results_Preprod.csv","w+") do |cs|
end
CSV.open("#{run_date}nteflix_Blind_Ingestion_issues.csv","w+") do |cs|
end
total_count = 0
total_movies = 0
$link_match_count = 0
...
results = collection.find({"service":"netflix","item_type":"movie"}).projection({"_id":0,"id":1,"title":1, "release_year":1})
results.each do |doc|
	doc = doc.to_json
	doc = JSON.parse(doc)
    arr << doc
...
p total_movies
#CSV.foreach("#{run_date}netflixdata.csv") do |row|
arr.each do |row|
    #p row
	nteflix_id = row["id"]
	puts "netflix dump videos are #{nteflix_id}"
	nteflix_title = row["title"]
	p nteflix_title
	nteflix_title = remove_special_ascii(nteflix_title);
	nteflix_title = nteflix_title.gsub(/\P{ASCII}/,'')
	#p nteflix_title
	nteflix_showtype = "MO"
	nteflix_releaseyear = row["release_year"]
	nteflix_releaseyear = nteflix_releaseyear.to_i    
    blind_ingest_status = []
    blind_ingest_status = check_blind_projectx_ingested(nteflix_id);
    if blind_ingest_status[0] == "Pass"
        blind_ingest_id = blind_ingest_status[1]
    	begin
    		oz_req = voice_search_pagination(nteflix_title,"ott_search")
    		...
    		if !oz_req.empty?
                oz_req.each do |res|
    				...
    				if res["object"]["show_type"] == "MO"
    					if (res["object"]["release_year"] == nteflix_releaseyear)
    						#p "rel year success..."
                            rlyr_flg = 1
    		                ...
    		                if aliases_array.include? nteflix_title
    		                    alias_match_flag = 1
    		                end
    						if (oz_title_mod == nteflix_title) || (oz_origtitle_mod == nteflix_title) || (alias_match_flag == 1)
    							flag = 1
    							$title_match_count = $title_match_count + 1
    							#p "...1..."
    							oz_netflix = Array.new
    							oz_nteflix_lid_arr = Array.new
    							oz_netflix << nteflix_title.downcase
    							oz_netflix << nteflix_releaseyear
    							oz_netflix << nteflix_id
    							oz_netflix << "200"
                                oz_netflix << "exact release year"
    							oz_netflix << "title matched"						
    							oz_netflix << oz_title.downcase
    							oz_netflix << res["object"]["release_year"]
                                mapped_id = res["object"]["id"]
                                oz_netflix << mapped_id
                                oz_netflix << blind_ingest_id
                                if mapped_id == blind_ingest_id
                                    oz_netflix << "Search result ID is same as Ingested ID"
                                else
                                    oz_netflix << "Search result ID is not same as Ingested ID"
                                end
    							res["object"]["videos"].each do |vid|
    								#p "...2..."
    								oz_nteflix_lid_arr << vid["launch_id"]
    							end
    							ret = process(nteflix_id,oz_nteflix_lid_arr);
    							oz_netflix += ret
    							CSV.open("#{run_date}nteflix_preprod.csv","a+") do |cs|
    								cs << oz_netflix
    							end
    							break
    						end									
                ...
                if rlyr_flg == 0
                    oz_req.each do |res|
                        ...
                        puts res["object"]["id"]
                        if res["object"]["show_type"] == "MO"
                            if (res["object"]["release_year"] == nteflix_releaseyear - 1 || res["object"]["release_year"] == nteflix_releaseyear + 1)
                                #p "rel year success..."
                                rlyr_flg = 1
                                oz_title = res["object"]["long_title"]
                                oz_origtitle = res["object"]["original_title"]                      
                                ...
                                if (oz_title_mod == nteflix_title) || (oz_origtitle_mod == nteflix_title) || (alias_match_flag == 1)
                                    ...
                                    oz_netflix << nteflix_id
                                    oz_netflix << "200"
                                    oz_netflix << "tentative release year"
                                    oz_netflix << "title matched"                      
                                    oz_netflix << oz_title.downcase
                                    oz_netflix << res["object"]["release_year"]
                                    mapped_id = res["object"]["id"]
                ...
                pjx_id = check_projectx_ingested(nteflix_id);
                final_result = []
                final_result << nteflix_title.downcase
                final_result << nteflix_releaseyear
                final_result << nteflix_id            
                final_result << "NA"
                final_result << "NA"
                final_result << "NA"
                ...
                CSV.open("#{run_date}nteflix_movies_empty_search_results_Preprod.csv","a+") do |cs|
                    cs << final_result
                end
            end
    		if flag == 0 && empty_search_results_flag == 0
    			...
    			oz_netflix << nteflix_id
    			oz_netflix << "200"
    			oz_netflix << "title didn't matched"
    			CSV.open("#{run_date}nteflix_preprod.csv","a+") do |cs|
    				cs << oz_netflix
    			end
    			CSV.open("#{run_date}movie_notmapped_preprod.csv","a+") do |cs|
    				cs << oz_netflix
    			end
    		end
        ...
        oz_netflix << nteflix_id
        oz_netflix << "200"
        oz_netflix << "Blind Ingestion Failure"
        CSV.open("#{run_date}nteflix_Blind_Ingestion_issues.csv","a+") do |cs|
            cs << oz_netflix
        end
        CSV.open("#{run_date}nteflix_preprod.csv","a+") do |cs|
            cs << oz_netflix
        ...
        oz_netflix << nteflix_id
        oz_netflix << "200"
        oz_netflix << "Blind Ingestion Failure_Duplicates"
        CSV.open("#{run_date}nteflix_preprod.csv","a+") do |cs|
            cs << oz_netflix
        end
        oz_netflix << blind_ingest_status[1]
        CSV.open("#{run_date}nteflix_Blind_Ingestion_issues.csv","a+") do |cs|
            cs << oz_netflix
        end
    else 
        ...
        oz_netflix << nteflix_id
        oz_netflix << "200"
        oz_netflix << "Blind Ingestion Failure_Errors"
        CSV.open("#{run_date}nteflix_preprod.csv","a+") do |cs|
            cs << oz_netflix
        end
        oz_netflix << blind_ingest_status[1]
