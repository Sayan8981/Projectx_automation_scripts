...
class disneyplus_program_duplicate_checking:
    ...
    def reverse_api_extract(self, link_details,source,show_type):
        ...
        for data in reverse_api_resp:
            if data.get("data_source")=='Rovi' and data.get("type")=='Program' and data.get("sub_type")=='ALL':
                if data.get("projectxId") not in self.px_id:      
                    self.px_id.append(data.get("projectxId"))
    ...
    def duplicate_checking_series(self, series_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        self.show_id=series_data.get("series_id").encode().encode("ascii","ignore")
        ...
        if self.series_title is not None:
            ...
            self.logger.debug ("projectx_ids : {},disneyplus_se_id: {}, disneyplus_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
                ...
                self.logger.debug ("projectx_ids_series : {0}".format(self.series_id_px),"thread:",thread_name) 
    ...
    def duplicate_checking_movies(self, movie_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        self.logger.debug ("projectx_ids : {}, disneyplus_mo_id: {}".format(self.px_id,self.id),"threads:",thread_name)
        if len(self.px_id) > 1:               
    ...
    #TODO: to get source id of disneyplus                      
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile, output_file_credit_match_false as mycsvfile1:
            ...
            for _id in range(start_id,end_id,100):
                try:
                    query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"disneyplus"}]}}
                    ...
                    self.logger.debug ("\n")
                    self.logger.debug({"start": start_id,"end":end_id})
                    for data in query:
                        series_title=''
                        _id_=data.get("id")
    ...
    def thread_pool(self):
        ...
        t1=Process(target=self.main,args=(0,"thread-1",9140))
        t1.start()


#starting
disneyplus_program_duplicate_checking().thread_pool()