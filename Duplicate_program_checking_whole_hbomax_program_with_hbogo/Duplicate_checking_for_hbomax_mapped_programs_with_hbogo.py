...
class headrun_hbomax_program_duplicate_checking:
    ...
    def duplicate_checking_series(self, series_data, writer,writer_credit_match_false,thread_name): 
        ...
        if self.series_title is not None:
            ...
            print ("projectx_ids : {},headrun_hbomax_se_id: {}, headrun_hbomax_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
                ...
                self.service='hbogo'
                self.reverse_api_extract(self.id,self.source,'SE')
                print ("projectx_ids : {},headrun_hbogo_se_id: {}, headrun_hbomax_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
                if len(self.px_id) > 1:
                    px_link=self.projectx_preprod_api%'{}'.format(",".join([str(data) for data in self.px_id]))
                    data_resp_link=lib_common_modules().fetch_response_for_api_(px_link,self.token)
                    for id_ in data_resp_link:
                        if id_.get("series_id") not in self.series_id_px:
                            self.series_id_px.append(id_.get("series_id"))
                    print ("projectx_ids_series hbogo: {0}".format(self.series_id_px),"thread:",thread_name)
                    if len(self.series_id_px)>1:
                        data_resp_search=duplicate_script_modules().search_api_call_response(self.series_title,self.projectx_preprod_search_api,self.projectx_domain,self.token)
                        if data_resp_search is not None:
                            result=duplicate_script_modules().search_api_response_validation(data_resp_search, self.source, self.series_id_px, duplicate,'SM',self.token,self.projectx_preprod_api,
                                    self.projectx_mapping_api,self.projectx_preprod_api,self.duplicate_api,self.credit_db_api)
                            if (self.credit_match=='False' or self.credit_match=='') and len(result["search_px_id"])==2:
                                #import pdb;pdb.set_trace()
                                px_link=self.projectx_preprod_api %'{}'.format(",".join([str(i) for i in result["search_px_id"]]))
                                data_resp_credits=lib_common_modules().fetch_response_for_api_(px_link,self.token)
    ...
    #TODO: to get source id of headrun_hbomax                      
    ...
    def thread_pool(self):
        ...
        t1=Process(target=self.main,args=(0,"thread-1",18000))
        t1.start()


#starting
if __name__=="__main__":
    headrun_hbomax_program_duplicate_checking().thread_pool()