...
class appletv_program_duplicate_checking:
    ...
    def cleanup(self):
        #import pdb;pdb.setrecursionlimit_trace()
        self.id = 0
        self.show_id=0
        self.series_title = ''
        self.episode_title=''
    ...
    def duplicate_checking_series(self, series_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=series_data.get("id").encode("utf-8")
        ...
        if self.series_title is not None:
            ...
            self.logger.debug ("projectx_ids : {},appletv_se_id: {}, appletv_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
    ...
    def duplicate_checking_movies(self, movie_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=movie_data.get("id").encode("utf-8")    
        ...
        self.logger.debug ("projectx_ids : {}, appletv_mo_id: {}".format(self.px_id,self.id),"threads:",thread_name)
        if len(self.px_id) > 1:               
    ...
    #TODO: to get source id of appletv                      
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile, output_file_credit_match_false as mycsvfile1:
            ...
            for _id in range(start_id,end_id,100):
                try:
                    query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"appletv"}]}}
...
#starting
appletv_program_duplicate_checking().thread_pool()