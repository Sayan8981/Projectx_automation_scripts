...
class hbomax_ingestion:
    ...
    def query_execute(self,_id):
        try:
            # query_headrun_hbomax=self.sourcetable.find({"service":self.source.lower(),"id":"urn:hbo:episode:GViPUWQ7rAp3Cw2oKAABf"})
            query_headrun_hbomax=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["tvshow","movie","episode"]}},{"service":"hbomax"}]}},{"$project":{"id":1,"_id":0,"item_type":1,"series_id":1,"title":1,"episode_title":1,"release_year":1,"episode_number":1,"season_number":1,"duration":1,"image_url":1,"url":1,"description":1,"cast":1,"directors":1,"writers":1,"categories":1,"genres":1,"expiry_date":1,"purchase_info":1,"service":1}},{"$skip":_id},{"$limit":100}])
            return query_headrun_hbomax
        except (MySQLError,IntegrityError) as e:
            self.logger.debug(['Got error {!r}, errno is {}'.format(e, e.args[0])])
            self.query_execute()    

    #TODO: to check hbomax_id ingestion
    def ingestion_checking(self,thread_name):
        duplicate_px_id=[]
        try:
            hbomax_mapping_api=self.source_mapping_api%(self.hbomax_id,self.source,self.show_type.encode())
            data_hbomax_resp=lib_common_modules().fetch_response_for_api_(hbomax_mapping_api,self.token)
            if data_hbomax_resp!=[]: 
                self.ingested_count+=1
                #TODO: checking program response in Programs_Id search api 
                for response in data_hbomax_resp:
                ...
                self.writer.writerow([self.hbomax_id,self.projectx_id,self.show_type,self.title,self.series_title,self.episode_title,self.hbomax_show_id,self.season_number,self.episode_number,self.year,self.duration,self.updated_at,self.expiry_date,'','','True','',self.px_response])
            # TODO : to check duplicate source_ids 
            else:   
                if self.show_type=='SM' or self.show_type=='MO':
                    hbomax_duplicate_api=self.source_duplicate_api%(self.hbomax_id,self.source,self.show_type.encode())
                else:
                    #TODO: for episodes
                    hbomax_duplicate_api=self.source_duplicate_api%(self.hbomax_show_id,self.source,'SM')    
                data_hbomax_resp_duplicate=lib_common_modules().fetch_response_for_api_(hbomax_duplicate_api,self.token)
                if data_hbomax_resp_duplicate!=[]:
                    self.ingested_count+=1
                    for px_id in data_hbomax_resp_duplicate:
                    ...
                    self.writer.writerow([self.hbomax_id,duplicate_px_id,self.show_type,self.title,self.series_title,self.episode_title,self.hbomax_show_id,self.season_number,self.episode_number,self.year,self.duration,self.updated_at,self.expiry_date,'True',source_id_duplicate,'True'])
                else:
                    self.not_ingested_count+=1
                    self.writer.writerow([self.hbomax_id,'',self.show_type,self.title,self.series_title,self.episode_title,self.hbomax_show_id,self.season_number,self.episode_number,self.year,self.duration,self.updated_at,self.expiry_date,'False','','','True',self.px_response]) 
            self.logger.debug('\n')
            self.logger.debug(["%s_id:"%self.source,self.hbomax_id,"show_type:",self.show_type,thread_name,"updated:","title:",self.title,"series_title: ",self.series_title,"season_no:",self.season_number,"episode_no:",self.episode_number,"ingested_count:",self.ingested_count,
            ...
            self.retry_count+=1
            self.logger.debug(["exception caught ingestion_checking func.........................",type(e),self.hbomax_id,self.show_type,thread_name])
    ...
    #TODO: getting source_ids from HBOMAXanytime_dump
    def getting_source_details(self,data):
        self.logger.debug("\n")
        self.logger.debug(["Checking ingestion of hbomax series, Movies,episodes in Projectx ........."])   
        self.hbomax_id=data["id"]
        self.hbomax_show_id=data["series_id"]
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile:
            ...
            for id_ in range(start_id,end_id,100):
                self.total+=1
                hbomax_data = self.query_execute(id_)
                for data in hbomax_data:
                    self.getting_source_details(data) 
                    if self.show_type is not None:   
                        self.ingestion_checking(thread_name)
                    else:
                        self.writer.writerow([self.hbomax_id,'',self.show_type,self.title,self.series_title,self.episode_title,self.hbomax_show_id,self.season_number,self.episode_number,self.year,self.duration,self.updated_at,self.expiry_date])         
                    self.logger.debug("\n")
