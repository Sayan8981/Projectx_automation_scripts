...
class headrun_netflix_ott_validation:
    ...
    def ott_checking(self,only_mapped_ids,thread_name,data):
        ...
        try:     
            if only_mapped_ids["source_flag"]=='True' or only_mapped_ids["source_flag"]=='True(Rovi+others)':
                ...
                print("\n")
                print ({"total_only_headrun_netflix_mapped":self.total,"%s_id"%self.source:self.headrun_netflix_id,
                ...
                if projectx_details!='Null':
                    if data.get("purchase_info")!="":
                        self.link_expired=lib_common_modules().link_expiry_check_(self.expired_api,self.projectx_domain,self.headrun_netflix_id,self.source.lower(),self.expired_token)
                        ott_validation_result=ott_meta_data_validation_modules().ott_validation(projectx_details,self.headrun_netflix_id)
                        self.writer.writerow([self.headrun_netflix_id,data.get("series_id"),projectx_id,self.show_type,projectx_details["px_video_link_present"],
                                  '',ott_validation_result,only_mapped_ids["source_flag"],'',self.link_expired])
                    else:
                        self.writer.writerow([self.headrun_netflix_id,data.get("series_id"),projectx_id,self.show_type,'','','',
                            only_mapped_ids["source_flag"],'purchase_info_null'])     
                else:
                    self.writer.writerow([self.headrun_netflix_id,data.get("series_id"),projectx_id,self.show_type,projectx_details,
                             '','',only_mapped_ids["source_flag"],'Px_response_null'])
            else:  
                self.total+=1          
                self.writer.writerow([self.headrun_netflix_id,data.get("series_id"),'',self.show_type,'','',only_mapped_ids,'Px_id_null'])               
            ...
            print("\n")
            print ("exception/error caught in ott_checking func...................",type(e),self.headrun_netflix_id,thread_name)
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile:
            fieldnames = ["%s_id"%self.source,"Series_id","Projectx_id","show_type","px_video_link_present","%s_link_present"%self.source,"ott_link_result","mapping","","Expired"]
            ...
            for _id in range(start_id,end_id):
                ...
                query_headrun_netflix=self.sourcetable.find({"service":"netflix","id":"%s"%str(self.headrun_netflix_id)})  
                for data in query_headrun_netflix:
                    if data.get("id")!="":
                        ...
                        print ({"count_headrun_netflix_id":self.count_headrun_netflix_id,
                               "id":self.headrun_netflix_id,"thread_name":thread_name})
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(str(self.headrun_netflix_id),self.source_mapping_api,
    ...
    def threading_pool(self):    
        ...
        t1.start()
        t2=Process(target=self.main,args=(1000,"thread-2",1919))
        t2.start()
        # t3=Process(target=self.main,args=(8000,"thread-3",12000))
        # t3.start()
        # t4=Process(target=self.main,args=(12000,"thread-4",16000))
        # t4.start()
        # t5=Process(target=self.main,args=(16000,"thread-5",20000))
        # t5.start()
        # t6=Process(target=self.main,args=(20000,"thread-6",22504))
        # t6.start()
