...
class source_meta_data:
    ...
    def getting_source_details(self,headrun_usanetwork_id,show_type,source,thread_name,details,sourcetable):
        #import pdb;pdb.set_trace()
        headrun_usanetwork_credit_present='Null'
        headrun_usanetwork_genres=[]
        headrun_usanetwork_duration=''
        headrun_usanetwork_title=''
        headrun_usanetwork_episode_title=''
        headrun_usanetwork_description=''
        headrun_usanetwork_alternate_titles=[]
        headrun_usanetwork_release_year=0
        headrun_usanetwork_link_present='Null'
        try:
            headrun_usanetwork_credits=self.getting_credits(details)
            if headrun_usanetwork_credits:
                headrun_usanetwork_credit_present='True'
            try:        
                headrun_usanetwork_title=unidecode.unidecode(pinyin.get(details.get("title")))
            except Exception:
                pass
            try:        
                headrun_usanetwork_description=unidecode.unidecode(pinyin.get(details.get("description")).lower())
            except Exception:
                pass

            headrun_usanetwork_show_id=details.get("series_id")

            if details.get("genres")!="":
                headrun_usanetwork_genres.extend(json.loads(details.get("genres").lower()))

            headrun_usanetwork_link=unidecode.unidecode(pinyin.get(details.get("url")))
            if headrun_usanetwork_link!="" or headrun_usanetwork_link is not None :
                headrun_usanetwork_link_present='True' 
            if details.get("release_year")!="":
                headrun_usanetwork_release_year=details.get("release_year").encode("ascii","ignore")
            #import pdb;pdb.set_trace()

            headrun_usanetwork_duration=details.get("duration")
            if headrun_usanetwork_duration is None or headrun_usanetwork_duration=="0":
                headrun_usanetwork_duration='0'

            #import pdb;pdb.set_trace()
            if show_type=="SE":
                headrun_usanetwork_title=unidecode.unidecode(pinyin.get(details.get("episode_title")))
            headrun_usanetwork_episode_number=details.get("episode_number").encode("ascii","ignore")
            if headrun_usanetwork_episode_number=="":
                headrun_usanetwork_episode_number=0
            headrun_usanetwork_season_number=details.get("season_number").encode("ascii","ignore")
            if headrun_usanetwork_season_number =="":
                headrun_usanetwork_season_number=0

            headrun_usanetwork_images_details=self.getting_images(headrun_usanetwork_id,show_type,headrun_usanetwork_show_id,details,sourcetable)

            return {"source_credits":headrun_usanetwork_credits,"source_credit_present":headrun_usanetwork_credit_present,"source_title":headrun_usanetwork_title,"source_description":headrun_usanetwork_description,
            "source_genres":filter(None,headrun_usanetwork_genres),"source_alternate_titles":headrun_usanetwork_alternate_titles,"source_release_year":headrun_usanetwork_release_year,"source_duration":headrun_usanetwork_duration,
            "source_season_number":headrun_usanetwork_season_number,"source_episode_number":headrun_usanetwork_episode_number,"source_link_present":headrun_usanetwork_link_present,"source_images_details":headrun_usanetwork_images_details}    
        except (httplib.BadStatusLine,Exception,urllib2.HTTPError,socket.error,urllib2.URLError,RuntimeError):
            self.retry_count+=1
            print ("exception caught getting_source_details func..............",headrun_usanetwork_id,show_type,source,thread_name)
            print ("\n") 
            print ("Retrying.............",self.retry_count)
            print ("\n")    
            if self.retry_count<=5:
                self.getting_source_details(headrun_usanetwork_id,show_type,source,thread_name,details,sourcetable)    
            else:
                self.retry_count=0

#main class
class mata_data_validation_headrun_usanetwork:

    def __init__(self):
        self.source="UsaNetwork"
        self.token='Token token=efeb15f572641809acbc0c26c9c1b63f4f7f1fd7dcb68070e45e26f3a40ec8e3'
        self.expired_token='Token token=0b4af23eaf275daaf41c7e57749532f128660ec3befa0ff3aee94636e86a43e7'
        self.total=0
        self.count_headrun_usanetwork_id=0
        self.writer=''
        self.link_expired=''
    ...
    def executed_query(self,skip_id):
        query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":[ "movie","episode","tvshow" ]}}
            ...
            ,"purchase_info":1,"service":1}},{"$skip":skip_id},{"$limit":10}])
        return query        
    ...
    def meta_data_validation_(self,data,projectx_id,thread_name,only_mapped_ids):
        #import pdb;pdb.set_trace()
        source_details=source_meta_data().getting_source_details(self.headrun_usanetwork_id,self.show_type,self.source,thread_name,data,self.sourcetable)
        ...
        meta_data_validation_result=ott_meta_data_validation_modules().meta_data_validate_headrun().meta_data_validation(self.headrun_usanetwork_id,source_details,projectx_details,self.show_type)
        ...
        try:
            if projectx_details!='Null':
                self.writer.writerow([self.headrun_usanetwork_id,projectx_id,self.show_type,source_details["source_title"],projectx_details["px_long_title"],projectx_details["px_episode_title"],meta_data_validation_result["title_match"],meta_data_validation_result["description_match"],meta_data_validation_result["genres_match"]
                ...
                self.writer.writerow([self.headrun_usanetwork_id,projectx_id,self.show_type,'','','','','',''
                ,'','','','','','','','','','','','','Px_response_null'])    
        except Exception as e:
            print ("get exception in meta_data_validation func........",type(e))
            pass                

    #TODO: Getting_projectx_ids which is only mapped to headrun_usanetwork
    def main(self,start_id,thread_name,end_id):
        #import pdb;pdb.set_trace()
        self.get_env_url()
        self.mongo_mysql_connection()
        result_sheet='/result/headrun_usanetwork_meta_data_checking%s_%s.csv'%(thread_name,datetime.date.today())
            ...
            for _id in range(start_id,end_id,10):
                ...
                for data in query_headrun_usanetwork:
                    if data.get("id")!="":
                        ...
                        print ("\n")
                        print("%s_id:"%self.source,"id:",self.headrun_usanetwork_id,"count_headrun_usanetwork_id:"
                        +str(self.count_headrun_usanetwork_id),"show_type:",self.show_type,"name:"+str(thread_name))
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(self.headrun_usanetwork_id,self.source_mapping_api
                        ...
                        if only_mapped_ids["source_flag"]=='True':
                            ...
                            print("\n")
                            print ({"total":self.total,"id":self.headrun_usanetwork_id,"Px_id":projectx_id,
    ...
    def threading_pool(self):    
        ...
        t1.start()
        t2=Process(target=self.main,args=(200,"thread-2",470))
        t2.start()

    #starting     
mata_data_validation_headrun_usanetwork().threading_pool()
