...
class hulu_link_checking_from_sheet_input:
    ...
    def getting_input_from_sheet(self,input_data,id_):
        ...
        self.hulu_id=str(input_data[id_][0])
    ...
    def main(self,start_id,thread_name,end_id):
        #import pdb;pdb.set_trace()
        inputFile="input/hulu_link_mising_sheet"
        ...
        self.column_fieldnames=["hulu_id","launch_id","Projectx_id","Preprod_link_status","Link_fetched_from","Fetched_from_sources","link_expired","Comment"]
        with output_file as mycsvfile:
            ...
            for id_ in range(start_id,end_id):
                ...
                self.total+=1
                self.link_expired=lib_common_modules().link_expiry_check_(self.expired_api,self.prod_domain,self.hulu_id,'hulu',self.expired_token)
                ...
                if self.preprod_videos_response[0]:
                    ...
                    if self.preprod_launch_id or self.preprod_video_link: 
                        ...
                        for source in self.fetch_from:
                            ...
                            elif ('Rovi' in self.fetch_from and 'hulu' in self.fetch_from) and len(self.fetch_from)>1:
                                self.fetched_source='Rovi+others'
                                self.comment='Pass'
                            elif ('Rovi' not in self.fetch_from or 'hulu' not in self.fetch_from) and len(self.fetch_from)>1:
                ...
                self.logger.debug("\n")  
                self.logger.debug([{"hulu_id":self.hulu_id,"link_id":self.link_id,"px_id":self.projectx_id,
                ...
                self.writer.writerow([self.hulu_id,self.link_id,self.projectx_id,self.preprod_link_status,self.fetched_source,self.fetch_from,self.link_expired,
                    self.comment])
        output_file.close() 
...
if __name__ == "__main__": 
    ...
    object_=hulu_link_checking_from_sheet_input()
    object_.get_env_url()
    object_.thread_pool()