...
class starz_program_duplicate_checking:
    ...
    def duplicate_checking_series(self, series_data, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=series_data.get("source_program_id")
        self.show_id=series_data.get("series_id")
        ...
        if self.series_title is not None:
            ...
            print ("projectx_ids : {},starz_se_id: {}, starz_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
    ...
    def duplicate_checking_movies(self, movie_data, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=movie_data.get("source_program_id")
        ...
        print ("projectx_ids : {}, starz_mo_id: {}".format(self.px_id,self.id),"threads:",thread_name)
        if len(self.px_id) > 1:               
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile, output_file_credit_match_false as mycsvfile1:
            ...
            for _id in range(start_id,end_id,100):
                try:
                    query=self.cur.execute("SELECT * FROM starz_programs where item_type in ('movie','episode') limit %d,100"%(_id))
                    #query=self.cur.execute("SELECT * FROM starz_programs where (expired_at is null or expired_at > '%s') and launch_id in ('urn:hbo:episode:GXaoEcARMaKCkmwEAAAse') and show_type in ('MO','SE','OT') limit %d,100"%(self.running_datetime,_id))
                    print ("\n")
                    print({"start": start_id,"end":end_id})
                    query_result=self.cur.fetchall()
                    for data in query_result:
                        series_title=''
                        _id_=data.get("source_program_id")
                        sm_id=data.get("series_id")
                        show_type=data.get("item_type")
                        show_type='MO' if show_type=='movie' else show_type
                        show_type='SE' if show_type=='episode' else show_type
...
if __name__=="__main__":
    starz_program_duplicate_checking().thread_pool()
