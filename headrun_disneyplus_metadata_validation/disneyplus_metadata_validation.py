...
class source_meta_data:
    ...
    def getting_source_details(self,disneyplus_id,show_type,source,thread_name,details,sourcetable):
        #import pdb;pdb.set_trace()
        disneyplus_credit_present='Null'
        disneyplus_genres=[]
        disneyplus_duration=''
        disneyplus_title=''
        disneyplus_episode_title=''
        disneyplus_description=''
        disneyplus_alternate_titles=[]
        disneyplus_release_year=0
        disneyplus_link_present='Null'
        try:
            disneyplus_credits=self.getting_credits(details)
            if disneyplus_credits:
                disneyplus_credit_present='True'
            try:
                disneyplus_title=unidecode.unidecode(pinyin.get(details.get("title")))
            except Exception:
                pass
            try:
                disneyplus_description=unidecode.unidecode(pinyin.get(details.get("description")).lower())
            except Exception:
                pass

            disneyplus_show_id=details.get("series_id")

            if details.get("genres")!="":
                disneyplus_genres.extend(json.loads(details.get("genres").lower()))

            disneyplus_link=unidecode.unidecode(pinyin.get(details.get("url")))
            if disneyplus_link!="" or disneyplus_link is not None :
                disneyplus_link_present='True'
            if details.get("release_year")!="":
                disneyplus_release_year=details.get("release_year").encode("ascii","ignore")
            #import pdb;pdb.set_trace()

            disneyplus_duration=details.get("duration")
            if disneyplus_duration is None or disneyplus_duration=="0":
                disneyplus_duration='0'

            #import pdb;pdb.set_trace()
            if show_type=="SE":
                disneyplus_title=unidecode.unidecode(pinyin.get(details.get("episode_title")))
            disneyplus_episode_number=details.get("episode_number").encode("ascii","ignore")
            if disneyplus_episode_number=="":
                disneyplus_episode_number=0
            disneyplus_season_number=details.get("season_number").encode("ascii","ignore")
            if disneyplus_season_number =="":
                disneyplus_season_number=0

            disneyplus_images_details=self.getting_images(disneyplus_id,show_type,disneyplus_show_id,details,sourcetable)

            return {"source_credits":disneyplus_credits,"source_credit_present":disneyplus_credit_present,"source_title":disneyplus_title,"source_description":disneyplus_description,
            "source_genres":filter(None,disneyplus_genres),"source_alternate_titles":disneyplus_alternate_titles,"source_release_year":disneyplus_release_year,"source_duration":disneyplus_duration,
            "source_season_number":disneyplus_season_number,"source_episode_number":disneyplus_episode_number,"source_link_present":disneyplus_link_present,"source_images_details":disneyplus_images_details}
        except (httplib.BadStatusLine,Exception,urllib2.HTTPError,socket.error,urllib2.URLError,RuntimeError):
            self.retry_count+=1
            self.logger.debug ("exception caught getting_source_details func..............",disneyplus_id,show_type,source,thread_name)
            self.logger.debug ("\n")
            self.logger.debug ("Retrying.............",self.retry_count)
            self.logger.debug ("\n")
            if self.retry_count<=5:
                self.getting_source_details(disneyplus_id,show_type,source,thread_name,details,sourcetable)
            else:
                self.retry_count=0

#main class
class mata_data_validation_disneyplus:
    ...
    #TODO: Getting_projectx_ids which is only mapped to disneyplus
    def main(self,start_id,thread_name,end_id):
        #import pdb;pdb.set_trace()
        self.get_env_url()
        self.mongo_mysql_connection()
        result_sheet='/result/disneyplus_meta_data_checking%s_%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as mycsvfile:
            ...
            for _id in range(start_id,end_id,1000):
                query_disneyplus=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":[ "movie","episode","tvshow" ]}},{"service":"disneyplus"}]}}
                ...
                #query_disneyplus=self.sourcetable.find({"service":"netflix","item_type":"episode","id":"80192967"})
                for data in query_disneyplus:
                    if data.get("id")!="":
                        #import pdb;pdb.set_trace()
                        disneyplus_id=data.get("id")
                        ...
                        self.logger.debug ("\n")
                        self.logger.debug("%s_id:"%self.source,"id:",disneyplus_id,"count_disneyplus_id:"
                            +str(self.count_disneyplus_id),"name:"+str(thread_name))
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(disneyplus_id,self.source_mapping_api
                        ...
                        if only_mapped_ids["source_flag"]=='True':
                            ...
                            self.logger.debug("\n")
                            self.logger.debug ({"total":self.total,"id":disneyplus_id,"Px_id":projectx_id,
    ...
    #starting
mata_data_validation_disneyplus().threading_pool()
