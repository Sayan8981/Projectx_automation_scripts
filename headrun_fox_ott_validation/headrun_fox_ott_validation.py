...
class fox_ott_validation:
    ...
    def getting_source_details(self,fox_id,show_type,details):
        #import pdb;pdb.set_trace()
        fox_link_present='Null'
        fox_id=details.get("id")
        fox_link=details.get("url").encode("ascii","ignore")
        if fox_link!="" or fox_link is not None :
            fox_link_present='True' 

        return {"source_link_present":fox_link_present,"source_id":fox_id,"show_type":show_type}  
    ...
    def ott_checking(self,only_mapped_ids,fox_id,show_type,thread_name,data):
        #import pdb;pdb.set_trace()
        try:     
            if only_mapped_ids["source_flag"]=='True':
                ...
                print("\n")
                print ({"total_only_fox_mapped":self.total,"%s_id"%self.source:fox_id,
                ...
                self.writer.writerow([fox_id,'',show_type,'','',only_mapped_ids,'Px_id_null'])          
            ...
            print("\n")
            print ("exception/error caught in ott_checking func.........................",type(e),fox_id,show_type,thread_name)
            if self.retry_count<=5:
                self.ott_checking(only_mapped_ids,fox_id,show_type,thread_name,data)  
            else:
                self.retry_count=0
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        result_sheet='/result/fox_ott_%s_checking%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as mycsvfile:
            ...
            for _id in range(start_id,end_id,100):
                print({"skip":_id})
                query_fox=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"fox"}]}}
                ...
                #query_fox=self.sourcetable.find({"service":"netflix","item_type":"movie","id":"70301275"})  
                for data in query_fox:
                    if data.get("id")!="":
                        #import pdb;pdb.set_trace()
                        fox_id=data.get("id").encode("ascii","ignore")
                        ...
                        print("\n")
                        print ({"count_fox_id":self.count_fox_id,
                                        "thread_name":thread_name})
                        #import pdb;pdb.set_trace()
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(str(fox_id),self.source_mapping_api,
                        	                                                     self.projectx_mapping_api,show_type,self.source,self.token)
                        self.ott_checking(only_mapped_ids,fox_id,show_type,thread_name,data)              
            print("\n")                    
            print ({"count_fox_id":self.count_fox_id,"name":thread_name})  
        output_file.close()                      
    ...
    #Starting    
fox_ott_validation().threading_pool()
