...
class deadsystem_content_validation:
    ...
    #db query
    def query(self,skip_id): 
        try:
            self.query_result=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"dump_date":self.current_date}]}},{"$skip":skip_id},{"$limit":100}])   
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        self.table.column_headers = ["Total test", "Test Pass", "Test Fail", "Code Exception/error"]
                    ...
                    try:
                        if data["source_id"] in self.source_array:
                            ...
                            if self.expiry_check == 'True':
                                ...
                                self.fail_count += 1
                            self.logger.debug("\n")    
                            self.logger.debug([[self.service_name,self.url,self.source_id,self.episode_title,self.title,self.item_type,self.release_year,self.season_number,self.episode_number,self.expiry_date,self.is_valid,self.modified_at,self.created_at,self.expiry_check,self.result],thread_name,{"pass":self.pass_count},{"fail":self.fail_count},{"total":self.total_count}])    
                        ...
                        self.logger.debug([{"error":error},{"data":data}]) 
                        pass    

            self.table.append_row([self.total_count, self.pass_count, self.fail_count ,self.code_exception])
    ...
    def threading_pool(self):    
        ...
        t1 = Process(target=self.main, args=(0,"thread-1",500))
        t1.start()
        t2 = Process(target=self.main, args=(500,"thread-2",1000))
        t2.start()
