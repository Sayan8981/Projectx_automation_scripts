...
class hbogo_ott_validation:
        ...
        try:     
            if only_mapped_ids["source_flag"]=='True' or only_mapped_ids["source_flag"]=='True(Rovi+others)':
                ...
                if projectx_details!='Null':
                    ...
                    self.writer.writerow([self.hbogo_id,projectx_id,self.show_type,projectx_details["px_video_link_present"],
                ...
                self.total+=1            
                self.writer.writerow([self.hbogo_id,'',self.show_type,'','',only_mapped_ids,'Px_id_null'])              
    ...
    def threading_pool(self):    
        ...
        t2.start()
        t3=Process(target=self.main,args=(1000,"thread-3",2000))
        t3.start()
        t4=Process(target=self.main,args=(2000,"thread-4",2481))
        t4.start()
