...
class appletv_ott_validation:
    ...
    def getting_source_details(self,appletv_id,show_type,details):
        #import pdb;pdb.set_trace()
        appletv_link_present='Null'
        appletv_id=details.get("id")
        appletv_link=details.get("url").encode("ascii","ignore")
        if appletv_link!="" or appletv_link is not None :
            appletv_link_present='True' 

        return {"source_link_present":appletv_link_present,"source_id":appletv_id,"show_type":show_type}  
            ...
            for data in data_px_resp:
                ...
                if px_video_link:
                    ...
                    for linkid in px_video_link:
                        #if linkid.get("source_id")=='appletv':
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        result_sheet='/result/appletv_ott_%s_checking%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as mycsvfile:
            ...
            for _id in range(start_id,end_id,100):
                ...
                query_appletv=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"appletv"}]}}
                ...
                #query_appletv=self.sourcetable.find({"service":"netflix","item_type":"movie","id":"70301275"})  
                for data in query_appletv:
                    if data.get("id")!="":
                        #import pdb;pdb.set_trace()
                        appletv_id=data.get("id").encode("ascii","ignore")
                        ...
                        self.logger.debug("\n")
                        self.logger.debug ({"count_appletv_id":self.count_appletv_id,
                                        "thread_name":thread_name})
                        #import pdb;pdb.set_trace()
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(str(appletv_id),self.source_mapping_api,
                        	                                                     self.projectx_mapping_api,show_type,self.source,self.token)
                        self.ott_checking(only_mapped_ids,appletv_id,show_type,thread_name,data)              
            self.logger.debug("\n")                    
            self.logger.debug ({"count_appletv_id":self.count_appletv_id,"name":thread_name})  
        output_file.close()                      
    ...
    #Starting    
appletv_ott_validation().threading_pool()
