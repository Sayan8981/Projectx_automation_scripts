...
class headrun_netflix_program_duplicate_checking:
    ...
    def duplicate_checking_series(self, series_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        self.show_id=eval(series_data.get("series_id").encode().encode("ascii","ignore"))
        ...
        if self.series_title is not None:
            ...
            print ("projectx_ids : {},headrun_netflix_se_id: {}, headrun_netflix_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
    ...
    def duplicate_checking_movies(self, movie_data, source, writer,writer_credit_match_false,thread_name): 
        ...
        print ("projectx_ids : {}, headrun_netflix_mo_id: {}".format(self.px_id,self.id),"threads:",thread_name)
        if len(self.px_id) > 1:               
    ...
    #TODO: to get source id of headrun_netflix                      
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile, output_file_credit_match_false as mycsvfile1:
            ...
            for _id in range(start_id,end_id,100):
                try:
                    query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"netflix"}]}}
    ...
    def thread_pool(self):
        ...
        t1=Process(target=self.main,args=(0,"thread-1",1))
        t1.start()
...
#starting
headrun_netflix_program_duplicate_checking().thread_pool()