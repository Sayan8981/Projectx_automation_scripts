...
class headrun_usanetwork_ott_validation:

    retry_count=0
    def __init__(self):
        self.source="Usanetwork"
        self.service="usanetwork"
    ...
    def executed_query(self,skip_id):
        query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"%s"%self.service}]}}
                        ...
                        "categories":1,"genres":1,"maturity_ratings":1,"purchase_info":1,"service":1}},{"$skip":skip_id},{"$limit":10}])    
        return query
    ...
    def ott_checking(self,only_mapped_ids,thread_name,data):
        ...
        try:     
            if only_mapped_ids["source_flag"]=='True' or only_mapped_ids["source_flag"]=='True(Rovi+others)':
                ...
                print("\n")
                print ({"total_only_headrun_usanetwork_mapped":self.total,"%s_id"%self.source:self.headrun_usanetwork_id,
                ...
                if projectx_details!='Null':
                    if data.get("purchase_info")!="":
                        self.link_expired=lib_common_modules().link_expiry_check_(self.expired_api,self.projectx_domain,self.headrun_usanetwork_id,self.service,self.expired_token)
                        ott_validation_result=ott_meta_data_validation_modules().ott_validation(projectx_details,self.headrun_usanetwork_id)
                        self.writer.writerow([self.headrun_usanetwork_id,projectx_id,self.show_type,projectx_details["px_video_link_present"],
                    ...
                    self.writer.writerow([self.headrun_usanetwork_id,projectx_id,self.show_type,projectx_details,'',
                             '','',only_mapped_ids["source_flag"],'Px_response_null'])
            else: 
                self.total+=1           
                self.writer.writerow([self.headrun_usanetwork_id,'',self.show_type,'','','',only_mapped_ids,'Px_id_null'])          
            ...
            print("\n")
            print ("exception/error caught in ott_checking func................",type(e),self.headrun_usanetwork_id,self.show_type,thread_name)
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        result_sheet='/result/headrun_usanetwork_ott_%s_checking%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as mycsvfile:
            ...
            for _id in range(start_id,end_id,10):
                ...
                for data in query_headrun_usanetwork:
                    if data.get("id")!="":
                        #import pdb;pdb.set_trace()
                        self.headrun_usanetwork_id=data.get("id").encode("ascii","ignore")
                        ...
                        print("\n")
                        print ({"count_headrun_usanetwork_id":self.count_headrun_usanetwork_id,
                        "show_type":self.show_type,"thread_name":thread_name,"nbc_id":self.headrun_usanetwork_id})
                        #import pdb;pdb.set_trace()
                        only_mapped_ids=ott_meta_data_validation_modules().getting_mapped_px_id_mapping_api(str(self.headrun_usanetwork_id),self.source_mapping_api,
                        	                                                     self.projectx_mapping_api,self.show_type,self.source,self.token)
                        self.ott_checking(only_mapped_ids,thread_name,data)              
            print("\n")                    
            print ({"count_headrun_usanetwork_id":self.count_headrun_usanetwork_id,"name":thread_name})  
        output_file.close()                      
    ...
    #Starting    
headrun_usanetwork_ott_validation().threading_pool()
