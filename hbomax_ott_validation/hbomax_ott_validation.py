...
class headrun_HBOMAX_ott_validation:
            ...
            if only_mapped_ids["source_flag"]=='True' or only_mapped_ids["source_flag"]=='True(Rovi+others)':
                ...
                if projectx_details!='Null':
                    if data.get("purchase_info")!="":
                        ...
                        self.writer.writerow([self.headrun_HBOMAX_id,projectx_id,self.show_type,projectx_details["px_video_link_present"],
                    ...
                    self.writer.writerow([self.headrun_HBOMAX_id,projectx_id,self.show_type,projectx_details,
                             '','',only_mapped_ids["source_flag"],'Px_response_null'])
            else:  
                self.total+=1          
                self.writer.writerow([self.headrun_HBOMAX_id,'',self.show_type,'','',only_mapped_ids,'Px_id_null'])               
    ...
    def main(self,start_id,thread_name,end_id):
        ...
        result_sheet='/result/headrun_HBOMAX_ott_%s_checking%s.csv'%(thread_name,datetime.date.today())
        ...
        with output_file as mycsvfile:
            ...
            for _id in range(start_id,end_id,100):
                print({"skip":_id})
                query_headrun_HBOMAX=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"hbomax"}]}},{"$project":{"id":1,"_id":0,"item_type":1,"series_id":1,"title":1,"episode_title":1,"release_year":1,"episode_number":1,"season_number":1,"duration":1,"image_url":1,"url":1,"description":1,"cast":1,"directors":1,"writers":1,"categories":1,"genres":1,"maturity_ratings":1,"purchase_info":1,"service":1}},{"$skip":_id},{"$limit":100}])
...
if __name__=="__main__":
    headrun_HBOMAX_ott_validation().threading_pool()
