...
class headrun_showtime_program_duplicate_checking:
    ...
    def get_env_url(self):
        ...
        self.duplicate_api='http://preprod-projectx-api-545109534.us-east-1.elb.amazonaws.com/projectx/duplicate?sourceId=%d&sourceName=%s&showType=%s'
    ...
    def default_param(self):
        ...
        self.fieldnames_tag = ["source","sm_id","id","show_type","movie_title","series_title","episode_title",
           ...
           "%s_id count for duplicate"%self.source,"Hulu_id count","vudu_id_count","Mapped rovi_id for duplicate","Mapped Gb_id for duplicate","Mapped %s_id for duplicate"%self.source,"Mapped_hulu_id","Mapped_vudu_id","variant_parent_id_present","variant_parent_id","link_fetched_from",
           "Sources of link"]
        self.fieldnames_credit_match_false = ["px_id1","px_id1_show_type","px_id1_variant_parent_id","px_id1_is_group_language_primary","px_id1_record_language","px_id2","px_id2_show_type","px_id2_variant_parent_id","px_id2_is_group_language_primary","px_id2_record_language","px_id1_credits_null","px_id1_db_credit_present","px_id2_credits_null","px_id2_db_credit_present",
            "long_title_match","original_title_match","runtime_match","release_year_match","alias_title_match","credit_match","match_link_id","link_match","comment"]
    ...
    def duplicate_checking_series(self, series_data, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=eval(series_data.get("id"))
        self.show_id=eval(series_data.get("series_id").encode("ascii","ignore"))
        ...
        if self.series_title is not None:
            ...
            print ("projectx_ids : {},headrun_showtime_se_id: {}, headrun_showtime_SM_id: {}".format(self.px_id,self.id,self.show_id),"thread_name:",thread_name)
            if len(self.px_id) > 1:
    ...
    def duplicate_checking_movies(self, movie_data, writer,writer_credit_match_false,thread_name): 
        ...
        self.id=eval(movie_data.get("id"))
        ...
        print ("projectx_ids : {}, headrun_showtime_mo_id: {}".format(self.px_id,self.id),"threads:",thread_name)
        if len(self.px_id) > 1:               
    ...
    #TODO: to get source id of headrun_showtime                      
    def main(self,start_id,thread_name,end_id):
        ...
        with output_file as mycsvfile, output_file_credit_match_false as mycsvfile1:
            ...
            writer_credit_match_false = csv.DictWriter(mycsvfile1,fieldnames=self.fieldnames_credit_match_false,dialect="excel",lineterminator = '\n')
            writer_credit_match_false.writeheader() 
            for _id in range(start_id,end_id,100):
                try:
                    query=self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"showtime"}]}},{"$project":{"id":1,"_id":0,"item_type":1,"series_id":1,"title":1,"episode_title":1,"release_year":1,"episode_number":1,"season_number":1,"duration":1,"image_url":1,"url":1,"description":1,"cast":1,"directors":1,"writers":1,"categories":1,"genres":1,"maturity_ratings":1,"purchase_info":1,"service":1}},{"$skip":_id},{"$limit":100}])
                    #query= self.sourcetable.aggregate([{"$match":{"$and":[{"item_type":{"$in":["movie","episode"]}},{"service":"showtime","id":"80131281"}]}}])
                    print ("\n")
    ...
    def thread_pool(self):
        ...
        t1=Process(target=self.main,args=(0,"thread-1",3000))
        t1.start()


#starting
if __name__=="__main__":
    headrun_showtime_program_duplicate_checking().thread_pool()